...
export default function Header() {
  ...
  const [keyward, setKeyWard] = useState(search);
  const handleSearch = () => {
    navigate('/posts?search=' + keyward);
  }

  useEffect(() => {
    if (!search) setKeyWard('');
  }, [search]);
        ...
        <div className="flex items-center space-x-4">
          <div className="relative">
            <input
              type="text"
              placeholder="Search"
              className={`bg-slate-800 text-white rounded-full py-1.5 px-4 pl-10 focus:outline-none focus:ring-1 focus:ring-blue-500 transition-all ${isSearchOpen
